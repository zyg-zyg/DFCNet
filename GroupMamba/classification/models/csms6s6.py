...
class CrossScan_4(torch.autograd.Function):
    ...
    def backward(ctx, ys: torch.Tensor):
        ...
        B, C, H, W = ctx.shape
        L = H * W
        y = ys[:, 0].view(B, -1, H, W).transpose(dim0=2, dim1=3).contiguous().view(B, -1, L).flip(dims=[-1]).view(B, 1,
                                                                                                                  -1, L)
        return y.view(B, -1, H, W)
...
class CrossMerge_5(torch.autograd.Function):
    @staticmethod
    def forward(ctx, ys: torch.Tensor):
        B, K, D, H, W = ys.shape
        ctx.shape = (H, W)
...
class CrossMerge_6(torch.autograd.Function):
    @staticmethod
    def forward(ctx, ys: torch.Tensor):
        B, K, D, H, W = ys.shape
        ctx.shape = (H, W)
