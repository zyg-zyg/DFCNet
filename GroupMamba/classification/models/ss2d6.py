...
class SS2Dv2:
    def __initv2__(
        ...
        # self.in_proj1 = Linear(d_model, d_proj//2, bias=bias)
        ...
        self.out_act1 = nn.GELU() if self.oact else nn.Identity()
        self.fusion_gate = nn.Conv2d(in_channels=d_inner * 2, out_channels=d_inner, kernel_size=1)
    ...
    def forwardv2(self, x: torch.Tensor,f:torch.Tensor, CrossScan, CrossMerge, **kwargs):
        x = self.in_proj(x)
        # f = self.in_proj1(f)
        if not self.disable_z:
            x, z = x.chunk(2, dim=(1 if self.channel_first else -1))  # (b, h, w, d)
            # f, z1 = f.chunk(2, dim=(1 if self.channel_first else -1))  # (b, h, w, d)
        ...
        if self.with_dconv:
            ...
            f = self.conv2d1(f)  # (b, d, h, w)
        x = self.act(x)
        f = self.act1(f)

        y = self.forward_core(x, CrossScan=CrossScan, CrossMerge=CrossMerge)
        y1 = self.forward_core(f, CrossScan=CrossScan, CrossMerge=CrossMerge)
