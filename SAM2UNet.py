...
class Adapter(nn.Module):
    def __init__(self, blk) -> None:
        ...
        self.prompt_learn = nn.Sequential(
            nn.Conv2d(dim, 72, kernel_size=1),
            nn.GELU(),
            nn.Conv2d(72, 72, kernel_size=3, padding=1,groups=72),
            nn.GELU(),
            nn.Conv2d(72, dim, kernel_size=1),
            nn.GELU(),
        )
    def forward(self, x):
        x1 = x.permute(0, 3, 1, 2).contiguous()
        prompt = self.prompt_learn(x1)
        x1 = prompt.permute(0, 2, 3, 1).contiguous()
        promped = x + x1
